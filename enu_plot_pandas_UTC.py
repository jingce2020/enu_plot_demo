...
for ii in range(0,len(data_frame)): 
    ...
    data_frame.at[ii,'TIME']=date_t
    data_frame.at[ii,'TIMES']=date_t

#-------------------构造连续时间序列-------------------#
t_beg=data_frame.at[0,'TIMES']
t_end=data_frame.at[len(data_frame)-1,'TIMES']
t_list=pd.date_range(t_beg,t_end,freq='30S')

#-------------------修改Dataframe索引字段-------------------#
data_frame.set_index(['TIMES'],inplace=True)
...
for ii in t_list:
    if (ii in data_frame.index):
        ...
        if(data_frame.at[ii,'Q']==5):
            enu_t=tools.ecef2enu(xyz_array,xyz_mean_array,lat_mean_deg,lon_mean_deg)  # 逐历元XYZ-->ENU
...
#-------------------重新构造enu Dataframe-------------------#
enu_frame=data_frame[['TIME','E','N','U']]

#-------------------pandas画图-------------------#
# enu_frame.plot(kind='line',x='TIME',y='E',figsize=(10, 6),color='red',grid=True)
# enu_frame.plot(kind='line',x='TIME',y='N',figsize=(10, 6))
# enu_frame.plot(kind='line',x='TIME',y='U',figsize=(10, 6))
...
ax.scatter(enu_frame['TIME'],enu_frame['E'],s=6)
ax=fig.add_subplot(3,1,2)
ax.scatter(enu_frame['TIME'],enu_frame['N'],s=6)
ax=fig.add_subplot(3,1,3)
ax.scatter(enu_frame['TIME'],enu_frame['U'],s=6)
